 ...
 *  2. Injetar `ChangeDetectorRef` e `PlayerService` e passá-los ao `super()`
 *  3. Garantir que `@ViewChild('videoPlayer')` está no template com #videoPlayer
 *  4. Sobrescrever `onEnded()` se precisar de comportamento pós-fim (ex.: próximo episódio)
 */
@Directive()
export abstract class PlayerBaseService implements OnDestroy {
